  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <UiButton variant="primary" @click="$router.push('/reparations/new')">
    ...
    >
      <template #id="{ item }">
        <NuxtLink :to="`/reparations/${item.id}`" class="text-indigo-600 hover:text-indigo-800 font-mono text-sm">
          {{ item.id }}
        </NuxtLink>
      </template>
      ...
      <template #actions="{ item }">
        <div class="flex items-center justify-center gap-2">
          <NuxtLink :to="`/reparations/${item.id}`">
