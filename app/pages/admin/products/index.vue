...
function coverImage(item: { media?: Array<{ kind: string; url: string }> }) {
  return item.media?.find((m) => m.kind === 'image')?.url || null;
}

async function togglePublish(item: Product) {
  if (publishing.value.has(item.id)) return;
  publishing.value.add(item.id);
  try {
    const res = await $fetch(`/api/products/${item.id}/publish`, {
      method: 'PUT',
    ...
    publishing.value.delete(item.id);
  }
}
            ...
            <template v-else v-for="(item, index) in products" :key="item.id">
              ...
              >
                ...
                <td class="h-10 px-4 py-2">
                  ...
                  >
                    <div
                      class="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-md border border-gray-200 bg-gray-50"
                    >
                      <img
                ...
                <td class="h-10 px-4 py-2">
                  <div class="flex items-center justify-end gap-1">
                    <button
                      ...
                      :title="
                        item.published
                          ? t('products.publish.toggleOff')
                          : t('products.publish.toggleOn')
                      "
                      :disabled="publishing.has(item.id)"
                      @click="togglePublish(item)"
                    >
                      <Icon
                        :name="item.published ? 'lucide:globe' : 'lucide:globe-lock'"
                        class="h-4 w-4"
                      />
                    </button>
                    <AdminCopyProductLink :slug="item.slug" :published="item.published" />
                    <button
